...
function getDistances(origin, destination) {
    ...
    return new Promise((resolve, reject) => {
        const distances = modes.map(mode => {
            return fetch(
                `${URL_TRAVEL_MODE}${origin}&destinations=${destination}&mode=${mode}&key=${KEY_GMAPS_TRAVEL}`
            )
                .then(res => res.json())
                .then(distance => {
                    return (result = [
        ...
        Promise.all(distances)
            .then(distances => {
                ...
                resolve(
                    `${shortestMode} is the quickest method at ${shortestTime}, from ${origin} to ${destination}.`
                );
            })
            .catch(err => {
                reject(err);
